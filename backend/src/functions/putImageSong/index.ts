...
const baseHandler: Handler<any, GUIDPathParameters> = async (event) => {
    ...
    const key = `${currentSong.guid}/image${extname(multipart.filename)}` 
    const [uploadOneError, imageLocation] = await uploadOne(key, multipart.content, {ContentType: multipart.contentType})
    if(uploadOneError){
        return formatJSONResponse(StatusInternalServerError, uploadOneError.message)
    }
    
    const [putOneSongError, newSong] = await putOneSong(currentSong.guid, {...currentSong, image: imageLocation})
    if(putOneSongError){
